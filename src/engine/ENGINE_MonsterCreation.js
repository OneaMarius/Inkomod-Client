...
export const generateMonsterNPC = (entityClass, subclassKey = null, requestedRank = null) => {
    ...
    if (subclassKey) {
        ...
        const classCandidates = availableMonsters.filter(key => 
            DB_NPC_MONSTERS[key].classification.entityClass === entityClass
        );
    ...
    if (requestedRank !== null) {
        finalRank = Math.max(
            genParams.rankRange[0],
            Math.min(requestedRank, genParams.rankRange[1])
        );
    } else {
    ...
    return {
        ...
        logistics: {
            ...
            foodYield: foodYield,
            foodConsumption: logParams.foodConsumption || 0,
            entityMass: entityMass,
        },
        
        economy: {
            baseCoinValue: profile.economy?.baseCoinValue || calculatedBounty,
            lootTableId: lootTableId,
        },
        
        interactions: {
            actionTags: profile.interactions?.actionTags || ['Fight_Monster', 'Evade_Monster', 'Ignore'],
        },
    };
};