...
export const generateHumanNPC = (subclassKey, poiRank) => {
	...
	if (!profile)
		throw new Error(`Human Engine Error: Invalid subclass [${dbSafeKey}]`);
	...
	const classTags =
		DB_NPC_TAXONOMY.Human.classInteractions[profile.entityClass] || [];
	const subclassTags =
		DB_NPC_TAXONOMY.Human.subclassInteractions[dbSafeKey] || [];

	// Combinăm tagurile specifice din profil cu cele date de Clasă și Subclasă (fără duplicate)
	const combinedTags = [
		...new Set([...specificTags, ...classTags, ...subclassTags]),
	];
