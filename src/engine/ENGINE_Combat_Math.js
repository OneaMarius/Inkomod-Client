...
export const calculateHitProbabilities = (attacker, defender, combatConfig, attackerStance, defenderStance, isDefenderCreature = false) => {
    ...
    let chanceEvade = 0, chanceBlock = 0, chanceParry = 0;
    const hasArmour = defender.equipment?.hasArmour || false;
    ...
    if (isDefenderCreature) {
        chanceEvade = Math.max(pMod.baseEvadeChance, Math.floor(defender.stats.agi / pMod.evadeDefenderAgiDivisor) + defCI - Math.floor(attacker.stats.agi / pMod.evadeAttackerAgiDivisor));
        chanceBlock = Math.max(0, Math.floor(defender.stats.dr / pMod.blockDefenderDdrDivisor));
        chanceParry = 0; // Creatures do not parry
    } else {
        let evadeBonus = (!hasArmour ? combatConfig.encumbranceBonuses.noArmourEvade : 0) + 
        ...
        chanceEvade = Math.max(pMod.baseEvadeChance, Math.floor(defender.stats.agi / pMod.evadeDefenderAgiDivisor) + defCI - Math.floor(attacker.stats.agi / pMod.evadeAttackerAgiDivisor)) + evadeBonus;
    ...
    let rawCritThreat = pMod.baseCritChance + Math.floor(attacker.stats.int / pMod.critAttackerIntDivisor) + Math.floor(attacker.stats.str / pMod.critAttackerStrDivisor);
    ...
    if (isDefenderCreature) {
        ...
        if (hasArmour && hasHelmet) critConversionRate = combatConfig.vulnerabilityRates.fullProtection;
        else if (hasHelmet && !hasArmour) critConversionRate = combatConfig.vulnerabilityRates.helmetOnly;
        else if (hasArmour && !hasHelmet) critConversionRate = combatConfig.vulnerabilityRates.armourOnly;
    }
