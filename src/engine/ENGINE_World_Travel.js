...
const getRouteData = (currentNodeId, targetNodeId) => {
	return DB_LOCATIONS_GATES.find(
		(route) =>
			(route.gateZone1 === currentNodeId && route.gateZone2 === targetNodeId) || (route.gateZone2 === currentNodeId && route.gateZone1 === targetNodeId),
	);
};
...
export const calculateTravelApCost = (
    ...
    if (playerEntity.equipment.hasMount && playerEntity.equipment.mountItem) {
        // Retrieve innateAgi (from animal DB) or standard agi, fallback to 5
        const mountAgi = playerEntity.equipment.mountItem.stats?.innateAgi || playerEntity.equipment.mountItem.stats?.agi || 5;
    ...
    // Apply the mount reduction factor to the ENTIRE travel cost, including seasonal weather penalties
...
export const getAvailableRoutes = (playerEntity, currentNodeId, seasonModifier = 0) => {
	...
	const connectedRoutes = DB_LOCATIONS_GATES.filter((route) => route.gateZone1 === currentNodeId || route.gateZone2 === currentNodeId);

	connectedRoutes.forEach((route) => {
		const targetNodeId = route.gateZone1 === currentNodeId ? route.gateZone2 : route.gateZone1;
		const targetNode = getNodeData(targetNodeId);

		if (targetNode) {
			const totalApRequired = calculateTravelApCost(playerEntity, currentNodeId, targetNodeId, seasonModifier);

			const totalCoinRequired = (route.costCoin || 0) + (targetNode.costCoin || 0);

			// Check if player has enough rank to pass the gate
			const meetsRankRequirement = playerRank >= (route.minRank || 0);

			const isAccessible = playerAp >= totalApRequired && playerCoins >= totalCoinRequired && meetsRankRequirement;
...
export const executeTravel = (playerEntity, currentNodeId, targetNodeId, seasonModifier = 0) => {
	...
	if (playerRank < (routeData.minRank || 0)) {
		return { status: 'FAILED_INSUFFICIENT_RANK', updatedPlayer: playerEntity };
	}
	...
	if (playerEntity.equipment.hasMount && playerEntity.equipment.mountItem) {
		const hpPenaltyMount = apCost * (WORLD.SPATIAL.transit.mountTransitHpPenaltyPerAp || 2);
		playerEntity.equipment.mountItem.biology.hpCurrent -= hpPenaltyMount;

		if (playerEntity.equipment.mountItem.biology.hpCurrent <= 0) {
			const baseYield = playerEntity.equipment.mountItem.logistics?.foodYield || 10;
	...
	if (playerEntity.inventory.animalSlots && playerEntity.inventory.animalSlots.length > 0) {
		const hpPenaltyCaravan = apCost * (WORLD.SPATIAL.transit.caravanTransitHpPenaltyPerAp || 1);
