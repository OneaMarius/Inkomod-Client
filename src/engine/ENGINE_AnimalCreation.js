...
import { formatForDB, formatForUI } from '../utils/NameFormatter.js'; // <-- IMPORT NOU

/**
 * Instantiates an Animal NPC (excluding Mounts).
 * @param {string} entityClass - Mandatory. 'Domestic' or 'Wild'.
 * @param {string|null} subclassKey - Optional. Key matching DB_NPC_ANIMALS or DB_NPC_TAXONOMY.
...
export const generateAnimalNPC = (entityClass, subclassKey = null, requestedRank = null) => {
    ...
    if (subclassKey) {
        // APLICĂM FORMATTER-UL AICI: Ne asigurăm că stringul are underscore pt DB
        ...
        if (tempProfile.classification.entityClass !== entityClass) {
            throw new Error(`Animal Engine Error: Subclass [${dbSafeKey}] does not belong to Class [${entityClass}]`);
        }
        targetSubclass = dbSafeKey;
    } else {
        const classCandidates = availableAnimals.filter((key) => DB_NPC_ANIMALS[key].classification.entityClass === entityClass);
    ...
    // APLICĂM FORMATTER-UL PENTRU UI: Generăm numele lizibil ("Water Buffalo")
    ...
    return {
        ...
        classification: {
            ...
            entitySubclass: uiName, // Păstrăm formatul frumos și aici
