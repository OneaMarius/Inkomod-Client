 ...
 */
export const calculateSellPrice = (baseGoldValue, regionalExchangeRate, currentDurability = 100, maxDurability = 100, playerHonor = 0, playerCha = 1) => {
	const regionalSilverValue = convertGoldToSilver(baseGoldValue, regionalExchangeRate);
	const conditionModifier = currentDurability / maxDurability;

	const { baseTradeSellPct, tradeHonorFactor } = WORLD.ECONOMY.tradeMultipliers;
	const chaBonus = playerCha / 500; // CHA 50 = 0.10 (10% extra profit)

	const dynamicMultiplier = baseTradeSellPct + tradeHonorFactor * playerHonor + chaBonus;
...
export const executeSellTransaction = (playerEntity, itemPayload, quantity, regionalExchangeRate, targetInventoryCategory, physicalItemIndex = null) => {
	...
	if (physicalItemIndex !== null) {
		...
		revenue = calculateSellPrice(baseCost, regionalExchangeRate, curDur, maxDur, playerHonor, totalCha);
		...
		const unitRevenue = calculateSellPrice(baseCost, regionalExchangeRate, 100, 100, playerHonor, totalCha);
