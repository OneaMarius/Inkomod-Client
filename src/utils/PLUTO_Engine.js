import {
	PLUTO_Global_Grid,
	PLUTO_Global_Gates,
} from '../data/PLUTO_GOD_World_Data';
import { ODIN_Formulas } from './ODIN_Engine';
...
export const PLUTO_Formulas = {
		...
		return PLUTO_Global_Gates.find(
			(gate) =>
				(gate.Gate_Zone_1 === Current_World_ID &&
					gate.Gate_Zone_2 === Target_World_ID) ||
				(gate.Gate_Zone_2 === Current_World_ID &&
					gate.Gate_Zone_1 === Target_World_ID),
		);
	},
	...
	Calculate_Global_Transit_Cost: (gameState, Target_World_ID) => {
		...
		const Season_Bonus =
			gameState.time.currentSeasonModifier?.Transit_AP_Bonus || 0;

		const encumbranceData =
			ODIN_Formulas.Calculate_Mass_And_Encumbrance(gameState);
		const Encumbrance_Penalty = encumbranceData.encumbrancePenaltyAP;

		let Mount_Factor = 1.0;

		if (gameState.player.equipped.mount) {
			const mountAgi = gameState.player.equipped.mount.biology?.agi || 5;
			Mount_Factor = Math.max(0.25, 0.75 - mountAgi * 0.01);
		}
	...
	Get_Available_Routes: (gameState) => {
		...
		Connected_Gates.forEach((gate) => {
			...
			if (Target_Zone) {
				const Total_AP_Required =
					PLUTO_Formulas.Calculate_Global_Transit_Cost(
						gameState,
						Target_World_ID,
					);
				...
				const Is_Accessible =
					Player_AP >= Total_AP_Required &&
					Player_Coins >= Total_Coin_Required;
