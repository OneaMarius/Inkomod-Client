...
// Description: Utility to calculate the total probability and danger level of an event triggering based on location taxonomy and seasonal multipliers.
 ...
 */
export const calculateDangerLevel = (worldId, currentSeason = 'summer') => {
    // 1. Find the target zone in the database
    const targetZone = DB_LOCATIONS_ZONES.find((zone) => zone.worldId === worldId);

    if (!targetZone) {
        console.warn(`[Event System] worldId ${worldId} not found in DB_LOCATIONS_ZONES. Returning baseline danger 25%.`);
        return 25;
    }
    ...
    // Ensure season exists, fallback to summer
    const normalizedSeason = currentSeason.toLowerCase();
    const seasonMultiplier = danger.seasonMultiplier[normalizedSeason] !== undefined ? danger.seasonMultiplier[normalizedSeason] : 1.0;

    // 4. Calculate final danger level
    let rawDanger = (catDanger + classDanger + subclassDanger) * seasonMultiplier;

    // 5. Clamp the minimum and maximum boundaries (0% to 100%)
    const finalDanger = Math.max(0, Math.min(100, rawDanger));
