...
const ShopView = () => {
	...
	const { merchantName, merchantSubclass, shopSuffix } = useMemo(() => {
		...
		const name =
			targetNpc?.entityName || (isRepairShop ? 'Blacksmith' : 'Merchant');
		const subclass =
			targetNpc?.classification?.entitySubclass ||
			(isRepairShop ? 'Blacksmith' : 'Merchant');

		const suffixes =
			WORLD.ECONOMY.shopSuffixes[tradeTag] ||
			WORLD.ECONOMY.shopSuffixes.default;
		const randomSuffix =
			suffixes[Math.floor(Math.random() * suffixes.length)];

		return {
			merchantName: name,
			merchantSubclass: subclass,
			shopSuffix: randomSuffix,
		};
	}, [targetId, tradeTag, isRepairShop, gameState?.activeEntities]);

	const targetNpcForRank = gameState?.activeEntities?.find(
		(npc) => npc.entityId === targetId,
	);
	const npcRank =
		targetNpcForRank?.classification?.entityRank ||
		targetNpcForRank?.classification?.poiRank ||
		5;
