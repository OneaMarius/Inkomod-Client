...
const ShopView = () => {
	...
	const bonusDelta = tradeTag === 'Trade_Coin' ? clampedBonusDelta / 2 : clampedBonusDelta;
	...
	useEffect(() => {
		...
		try {
			...
			if (tradeTag === 'Trade_Weapon') {
				...
				for (let i = 0; i < count; i++) {
					const newLoot = generateLootItem(null, calculateRankFromEconomy(ecoLevel));
	...
	const getItemPrice = (item) => {
		...
		if (shopMode === 'BUY') return calculateBuyPrice(baseCost, regionalExchangeRate, playerHonor, totalCha, isIngot);
	...
	const getRawItemPrice = (item) => {
		...
		if (shopMode === 'BUY') return calculateBuyPrice(baseCost, regionalExchangeRate, 0, 0, isIngot);
	...
	// 1. Extract and generate name components
	const { merchantName, merchantSubclass, shopSuffix } = useMemo(() => {
		const targetNpc = gameState?.activeEntities?.find((npc) => npc.entityId === targetId);

		const name = targetNpc?.entityName || (isRepairShop ? 'Blacksmith' : 'Merchant');
		const subclass = targetNpc?.classification?.entitySubclass || (isRepairShop ? 'Blacksmith' : 'Merchant');

		const suffixes = WORLD.ECONOMY.shopSuffixes[tradeTag] || WORLD.ECONOMY.shopSuffixes.default;
		const randomSuffix = suffixes[Math.floor(Math.random() * suffixes.length)];

		return { merchantName: name, merchantSubclass: subclass, shopSuffix: randomSuffix };
	}, [targetId, tradeTag, isRepairShop, gameState?.activeEntities]);

	const targetNpcForRank = gameState?.activeEntities?.find((npc) => npc.entityId === targetId);
	const npcRank = targetNpcForRank?.classification?.entityRank || targetNpcForRank?.classification?.poiRank || 5;
...
<ShopHeaderInfo
				...
				merchantSubclass={merchantSubclass}
				shopSuffix={shopSuffix}
				shopMode={shopMode}
			...
			{isFoodShop && (
				<div className={`${styles.seasonalBanner} ${getSeasonalClass(seasonFoodPriceMult)}`}>
					<span className={styles.seasonLabel}>Season: {activeSeason.toUpperCase()}</span>
