...
const MapView = () => {
	...
	const availableRoutes = useMemo(() => {
		...
		// Pasăm modificatorul în loc de 0
		return getAvailableRoutes(gameState.player, currentLocationId, seasonApModifier);
	}, [gameState?.player, currentLocationId, gameState?.time?.activeSeason]);
	...
	const mapLines = useMemo(() => {
		return DB_LOCATIONS_GATES.map((gate) => {
			...
			if (node1 && node2 && node1.mapX !== undefined && node2.mapX !== undefined) {
				...
				return { id: gate.worldId, x1: node1.mapX, y1: node1.mapY, x2: node2.mapX, y2: node2.mapY, isActiveRoute, isInaccessibleRoute };
			}
			return null;
		}).filter(Boolean);
				...
				return (
					...
					>
						...
						<div className={styles.nodeInfoContainer}>
							{/* AP Cost Icon (Left) */}
							{isAdjacent && <div className={styles.apIcon}>{totalApCost}</div>}

							{/* Location Name (Center) */}
							<span className={styles.nodeLabel}>{zone.zoneName.replace(/_/g, ' ')}</span>

							{/* Economy Level Icon (Right) */}
							<div className={styles.economyIcon}>{zone.zoneEconomyLevel || 1}</div>
						</div>
					</div>
				);
			})}
