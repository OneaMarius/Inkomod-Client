	...
	return (
		...
		>
			...
			>
				...
				<div style={{ backgroundColor: '#111', padding: '15px', borderRadius: '4px', marginBottom: '20px' }}>
					...
					<div
						style={{
							...
							marginBottom: monthlyReportData.animalsSacrificed > 0 || monthlyReportData.healthEvents?.length > 0 ? '10px' : '0',
						}}
					>
				...
				{monthlyReportData.socialEvents &&
					monthlyReportData.socialEvents.length > 0 &&
					(() => {
						// 1. Evaluate array contents for specific engine string signatures
						const hasPromotion = monthlyReportData.socialEvents.some((msg) => msg.includes('advanced to Rank'));
						const hasDemotion = monthlyReportData.socialEvents.some((msg) => msg.includes('demoted to Rank'));

						// 2. Define baseline styling (Standard Reputation Update)
						let headerText = 'REPUTATION UPDATE';
						let boxStyle = {
						...
						// 3. Override styling if a rank change is detected
						if (hasPromotion) {
							headerText = 'RANK PROMOTION';
							boxStyle.backgroundColor = 'rgba(212, 175, 55, 0.1)';
							boxStyle.border = '1px solid var(--gold-primary)';
							headerColor = 'var(--gold-primary)';
						} else if (hasDemotion) {
							headerText = 'RANK DEMOTION';
							boxStyle.backgroundColor = 'rgba(211, 47, 47, 0.1)';
							boxStyle.border = '1px solid #d32f2f';
							headerColor = '#d32f2f';
						}

						// 4. Render the dynamic block
						return (
							<div style={boxStyle}>
								<h4 style={{ color: headerColor, margin: '0 0 5px 0', fontSize: '0.9rem' }}>{headerText}</h4>
								{monthlyReportData.socialEvents.map((msg, idx) => (
									<p
										key={idx}
										style={{ color: '#fff', fontSize: '0.85rem', margin: '4px 0', fontStyle: 'italic' }}
									>
										"{msg}"
									</p>
								))}
