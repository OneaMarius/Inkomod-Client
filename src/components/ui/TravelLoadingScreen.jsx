...
const TravelLoadingScreen = () => {
	...
	const hasMount = useGameState(
		(state) => state.gameState?.player?.equipment?.hasMount,
	);
	...
	if (isCivilizedZone) {
		activePathClass = isWinter
			? styles.pathCivilizedWinter
			: styles.pathCivilized;
	} else {
		activePathClass = isWinter
			? styles.pathUntamedWinter
			: styles.pathUntamed;
	}
