...
const ItemInfo = ({ item }) => {
	...
	const playerEquipment = useGameState(
		(state) => state.gameState?.player?.equipment,
	);
	const rer =
		useGameState(
	...
	const itemRank =
		item.classification?.itemTier || item.classification?.entityRank;
	const itemQuality =
		item.classification?.itemQuality ||
		item.classification?.entityQuality ||
		null;
	const itemType =
	...
	// Booleans for Category Routing
	const isMount = itemType === 'Mount';
	const isAnimal =
		item.classification?.entityCategory === 'Animal' && !isMount;
	const isEquipment =
		item.classification?.itemCategory === 'Equipment' ||
		['Weapon', 'Armor', 'Shield', 'Helmet'].includes(itemType);
	const isLoot = item.classification?.itemCategory === 'Loot';
	const isTrophy = item.classification?.itemCategory === 'Trophy';

	let equippedItem = null;
	if (isEquipment && playerEquipment) {
		const slotKey = `${itemType.toLowerCase()}Item`;
	...
	const transitConstants = WORLD.SPATIAL?.transit || {
		mountMaxReductionFactor: 0.2,
		mountMinReductionFactor: 0.9,
	...
	const carryCapacity =
		mountCarryWeight.base + strValue * mountCarryWeight.bonusPerStr;
	...
	const renderComparedStat = (
		...
		const displayDelta = Number.isInteger(delta)
			? delta
		...
		return (
			...
			>
				...
				<span
					style={{ color: color, fontSize: '1rem', fontWeight: 'bold' }}
				>
					({sign}
					{displayDelta}
					{suffix})
	...
	// Determine the formatted subtitle for the category
	const getDisplayCategory = () => {
		if (isLoot) return `${itemType} Loot`;
		...
		<>
			...
			{isOpen && (
				<div className={styles.modalOverlay} onClick={handleClose}>
					...
					>
						<h3
							className={`${styles.modalHeader} ${itemQuality ? `textQ${itemQuality}` : ''}`}
						>
							{itemName}
						</h3>
						...
						<div className={styles.modalCategory}>
							...
							>
								...
								{itemQuality && (
									<div className={`badgeCircle badgeQ${itemQuality}`}>
										Q{itemQuality}
									</div>
								)}
							</div>
								...
								<>
									<div className={styles.statBox}>
										...
										{renderComparedStat(
											...
											equippedItem,
											(eq) =>
												eq.stats?.innateStr || eq.stats?.str || 0,
										)}
									</div>
									<div className={styles.statBox}>
										<span className={styles.statBoxLabel}>
										...
										{renderComparedStat(
											...
											equippedItem,
											(eq) =>
												eq.stats?.innateAgi || eq.stats?.agi || 0,
										)}
									</div>
									<div className={styles.statBox}>
										<span className={styles.statBoxLabel}>
									...
									<div className={styles.statBox}>
										...
										{renderComparedStat(
											item.biology?.hpMax || 0,
											equippedItem,
											(eq) => eq.biology?.hpMax || 0,
										)}
									</div>
									<div className={styles.statBox}>
										<span className={styles.statBoxLabel}>Mass</span>
										{renderComparedStat(
											item.logistics?.entityMass || 0,
											equippedItem,
											(eq) => eq.logistics?.entityMass || 0,
											false,
											' kg',
										)}
									</div>
									<div className={styles.statBox}>
										...
										{renderComparedStat(
											item.logistics?.foodConsumption || 0,
											equippedItem,
											(eq) => eq.logistics?.foodConsumption || 0,
											true,
										)}
									</div>
									<div className={styles.statBox}>
										...
										{renderComparedStat(
											item.logistics?.foodYield || 0,
											equippedItem,
											(eq) => eq.logistics?.foodYield || 0,
										)}
									</div>
								</>
							)}
							...
							{isAnimal && (
								<>
									...
									<div className={styles.statBox}>
										<span className={styles.statBoxLabel}>Mass</span>
										<span className={styles.statBoxValue}>
											{item.logistics?.entityMass || 0} kg
										</span>
									</div>
									<div className={styles.statBox}>
										<span className={styles.statBoxLabel}>
							...
							{isLoot && (
								<>
									...
									<div className={styles.statBox}>
										...
										<span className={styles.statBoxValue}>
											{(item.economy?.baseCoinValue || 0) * rer} c
										</span>
									</div>
								</>
							)}
							...
							{isTrophy && (
								<>
									...
									<div className={styles.statBox}>
										...
										<span className={styles.statBoxValue}>
											{item.classification?.itemSubclass ||
												'Unknown'}
										</span>
									</div>
								</>
