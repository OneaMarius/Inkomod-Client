...
const CombatHudTop = ({
    ...
    useEffect(() => {
        ...
        setPlayerDmgPop(null); setEnemyDmgPop(null);
        setPlayerIconPop(null); setEnemyIconPop(null);
        setPlayerHpGlow(false);
        setPlayerStatusTag(null); setEnemyStatusTag(null);
        ...
        return () => { clearTimeout(triggerTimer); clearTimeout(cleanupTimer); };
    }, [visualEvents]);
        ...
        <div className={styles.hudTop}>
            ...
            <div className={styles.portraitBox}>
                <div style={{ position: 'relative' }}>
                    ...
                    {playerDmgPop && (
                        <div className={styles.floatingDamageContainer}>
                            <span className={getFloatingDamageData(playerDmgPop).cssClass}>
                ...
                <div className={`${styles.hpBarContainer} ${getHpBarClass(playerAnim, playerHpGlow)}`}>
                    ...
                    {playerWoundPercent > 0 && <div className={styles.hpBarWound} style={{ width: `${playerWoundPercent}%` }}></div>}
                    <span className={styles.hpBarText}>{player.biology.hpCurrent} / {player.biology.hpMax}</span>
                </div>

                <div className={styles.statusTagContainer}>
                    {playerStatusTag && (
                        <span className={`${styles.statusTag} ${styles['tag' + playerStatusTag.type]}`}>
            ...
            <div className={styles.vsIcon}>
                <span>VS</span>
                <button className={styles.statsBtn} onClick={() => setIsInfoModalOpen(true)}>Stats</button>
            </div>
            ...
            <div className={styles.portraitBox}>
                <div style={{ position: 'relative' }}>
                    ...
                    {enemyDmgPop && (
                        <div className={styles.floatingDamageContainer}>
                            <span className={getFloatingDamageData(enemyDmgPop).cssClass}>
                ...
                <div className={`${styles.hpBarContainer} ${getHpBarClass(enemyAnim, false)}`}>
                    ...
                    <span className={styles.hpBarText}>{enemy.biology.hpCurrent} / {enemy.biology.hpMax}</span>
                </div>

                <div className={styles.statusTagContainer}>
                    {enemyStatusTag && (
                        <span className={`${styles.statusTag} ${styles['tag' + enemyStatusTag.type]}`}>
                            {enemyStatusTag.text}
                        </span>
                    )}
                </div>
